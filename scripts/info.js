...
(function () {
  ...
  if (sl) {
    ...
    const tags = { camping: 'incluso', indoor: 'su prenotazione', offsite: 'fai-da-te' };

    sl.innerHTML = ed.sleep.options.map(o => `
      <div class="sleep-card kind-${o.kind}">
        ...
        <div class="sleep-tag">${tags[o.kind] || ''}</div>
      </div>
    `).join('');
  }
  ...
  if (cg) {
    ...
    const items = [
      ...
      { label: 'Telegram', value: c.telegram, href: `https://t.me/${c.telegram.replace('@','')}`, icon: '✈️' },
      { label: 'Matrix', value: c.matrix,  href: '#',                                    icon: '🔷' },
      { label: 'Mastodon', value: c.mastodon, href: '#',                                 icon: '🐘' }
    ];
    cg.innerHTML = items.map(it => `
